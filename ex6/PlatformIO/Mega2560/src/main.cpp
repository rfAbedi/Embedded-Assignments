...
void setup() {
  ...
  // queue
  dcSpeedQueue = xQueueCreate(5, sizeof(int));
  servoAngleQueue = xQueueCreate(5, sizeof(int));

  // Create task
  xTaskCreate(taskTemperature, "taskTemperature", 128, NULL, 2, NULL);
  xTaskCreate(taskSwitchButton, "taskSwitchButton", 128, NULL, 2, NULL);
  if (dcSpeedQueue != NULL && servoAngleQueue != NULL) {
    xTaskCreate(taskStep, "taskStep", 128, NULL, 1, NULL);
    xTaskCreate(taskDcMotor, "taskDcMotor", 128, NULL, 0, NULL);
    xTaskCreate(taskServo, "taskServo", 128, NULL, 0, NULL);
  }
}
